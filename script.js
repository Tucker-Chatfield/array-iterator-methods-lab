...
inventorNames = inventors.map(person => {
...
totalYearsLived = inventors.reduce((accumalator, inventor) => {
  let lifespan = inventor.passed - inventor.year;
  return accumalator + lifespan;
}, 0);
...
travelMethodCounts = travelMethods.reduce((accumalator, method) => {
  if (accumalator[method]) {
    accumalator[method]++;
  } else {
    accumalator[method] = 1;
  }
  return accumalator;
}, {});
